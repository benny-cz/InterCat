...
internal sealed class HoverCardPainter(IBrush surface, IBrush ink, IBrush mutedInk, IPen border)
{
    ...
    /// <summary>Card text is laid out once per text and width, not on every pointer move, and draws without allocating.</summary>
    private readonly PaneText texts = new(256);
    ...
    public Rect? Draw(DrawingContext context, HoverCard card, Point pointer, Size pane)
    {
        ...
        TextLayout title = Text(card.Title, 12, strong: true, textWidth);
        double bodyHeight = 0;
        ...
        title.Draw(context, new(x + Padding, top));
        top += title.Height + 4;
        for (int i = 0; i < card.Lines.Count; i++)
        {
            TextLayout line = Text(card.Lines[i], 11, strong: false, textWidth);
            line.Draw(context, new(x + Padding, top));
            top += line.Height;
        }
    ...
    private TextLayout Text(string text, double size, bool strong, double width) =>
        texts.Get(text, size, strong ? ink : mutedInk, width, lines: 3, TextTrimming.WordEllipsis);
}

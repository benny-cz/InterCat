...
partition the instance's admitted, timed records, and each row judges coverage from its own observed mechanisms. All
five rows are always drawn, so no row moves under a zoom; a row with nothing drawn is labelled "none", so its coverage
strip is not read as records. A row holds each record's **source-catalog** direction (§23): a connection attempt
...
List selection, scroll bars and tool tips keep the control theme's look in high contrast for now, and a pass with Windows' own high-contrast themes is owed on Windows.
...
§11.3's sharing presets followed: a metadata-only redacted report (revision 104) and a reopenable redacted session package (revision 106). An explicitly unredacted original evidence package remains open.
...
- **Still open:** opening a 1,000,000-record session projects its overview in 1.55 s. At 10,000,000 records that exceeds
  §12's reopen budget, which is S4's and IC-015's work.
...
1M- and 10M-row sessions. Each projection also still re-reads and re-hashes every segment it opens: 21 MB and 17 ms at
the end of this capture. A live session also keeps every superseded manifest, because `store-v1` keeps an unreferenced
file until asked to remove it. That was 297 manifests and 16 MB after 10 minutes, growing with the square of the chunk
count.
...
**Implement:** adaptive indices/tiles, the complete §12.1 overview pyramid, bounded graph layout, cache/queue budgets, rolling retention with pin semantics, the original evidence export preset (the redacted report and redacted session package presets exist, §11.3), accessibility polish, signed installer/binaries, update compatibility and support diagnostics. Keep payloads and raw sensitive events out of automatic diagnostic bundles.
...
Interactive/query readers additionally keep a TUNABLE: 64 MiB per-store cache of **verified immutable segment readers**, admitted by the published byte lengths of each segment plus the dictionaries that reader decoded. A hit is valid only when the selected manifest still names the exact segment dependency and every exact dictionary dependency; a generation change prunes entries it can no longer reach, including evidence a retention publication released. Readers are admitted while they fit and served uncached once the cache is full; a reachable reader is never evicted to make room. Until S4 every projection and query scans the whole session in the same order, and under such scans a recency (LRU) policy evicts exactly the readers the next scan reads first, so a session larger than the budget would hit nothing and still pay for the churn. Admitting only what fits keeps a stable share cached, and pruning frees room as compaction and retention replace segments; viewport-scoped reads (S4) are the point to revisit the policy. One-pass compaction and journal re-derivation deliberately bypass the cache. A cached reader is shared by concurrent queries, so anything it verifies lazily, such as a column's checksum on first read, is marked only after the check passes: another thread either sees the mark or checks the immutable bytes itself, and no thread reads a column it has not seen pass. The bound is per store, and a viewer holds one store per session: a live capture's writer store is also the store the window's queries read through. A viewer that keeps several sessions' stores open, so that returning to one hashes nothing again, lets only the session it opened last keep its readers; the others keep only what they verified. The 64 MiB figure is a payload-admission bound, not a claim about CLR object overhead: decoded strings, indexes and object headers still count against §12's 512 MiB analysis/cache process budget and must be measured at the scale gates. This cache avoids repeated file reads and segment/dictionary integrity work for carried immutable segments; it **does not satisfy S4** and never makes a whole-session scan an acceptable interactive-open algorithm.
...
| Verified segment-reader cache admission | 64 MiB of published segment + dictionary payload per open session; managed overhead remains inside the 512 MiB analysis/cache budget | Storage/query | §20.1, §12 |

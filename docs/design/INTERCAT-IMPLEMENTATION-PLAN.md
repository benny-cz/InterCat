...
- **The broker half of the preview.** `GetStatus` now carries a **live preview** (`contracts/broker-v1.md` §5.9):
  ...
  only, and its counts plus its unbinned records always equal the records it covers. The viewer half and its
  measurement follow; until they land, the budget row remains a miss.

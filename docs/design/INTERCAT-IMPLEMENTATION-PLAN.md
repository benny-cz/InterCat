...
A redacted sharing export (§11.3, M5) remains open.
...
Revision 105 audits the redacted normalized-session path against the actual store. `CommitReplacingDerived` carries
the current admitted journal, normalizer plan, coverage ledger and finalization marker by design. It must not be used
to create a package labeled redacted. A new package instead starts a fresh store with new session/capture identities,
builds a sanitized admitted journal that contains no original payload or source locator, derives new rows, dictionaries
and indices from that journal, and rewrites or omits each companion dependency under the same allowlist. Its final
inventory and every cross-file reference are scanned before publication, then the package is reopened and checked
as a recipient would. If the original-record drill-down cannot point only into sanitized evidence, it is unavailable
in that package; no reference may resolve back into the source session (I22).

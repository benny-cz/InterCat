...
process-owner lanes as of revision 122, and L2 draws exact source-direction rows as of revision 123. Every deeper
rung retains an exact focus count for the records E reads:
...
| Direction, only where the derivation supports it | Arrowhead in the graph; in the timeline, L2's source-direction row, and band side in §6.2's band regime | Direction word in the hover card | Hue; an undirected/unknown relation gets no invented arrow |

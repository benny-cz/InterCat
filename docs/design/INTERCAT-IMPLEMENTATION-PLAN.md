...
**Status:** reviewed product architecture and implementation blueprint, revision 19, 2026-09-22. M0 implementation has started; progress and measured limitations are tracked in `docs/IMPLEMENTATION-STATUS.md`. M0 must resolve the stated capture feasibility gates before dependent features are committed for delivery. This design document does not itself imply a capture benchmark or capability claim. Revision 19 freezes the broker response and connection-dispatch layer: typed bounded responses cover negotiation, capability review, effective preparation and lifecycle outcomes; Hello gates commands; duplicate in-flight correlations are refused; authenticated owner identity is injected rather than read from a request; and reviewed duration/storage/retention limits are part of prepared-plan identity. The OS-authenticated pipe host remains disabled.

...
**The timeline follows the rung too.** L0 draws mechanism lanes as of revision 119. The deeper-rung lanes in the
table's timeline column are not built yet. Until they are, each deeper rung's timeline draws in colour the records E
reads from that rung:
...
but does not yet render or interact with them; L1 lane virtualization and the higher-rung semantics remain open.
...
| Direction, only where the derivation supports it | Arrowhead in the graph, band side in the timeline | Direction word in the hover card | Hue; an undirected/unknown relation gets no invented arrow |
...
| Arrow keys | Left/right pan by TUNABLE: 10% of the span; with `Shift`, by one cell. At L0, up/down scroll mechanism lanes, with Page Up/Down scrolling a page |
| `Home` / `End` | Go to the retained extent's edges |
| `0` | Fit the current analysis scope |
| `[` / `]` | Previous / next observation in the selected lane or channel is the target behavior. At L0 a selected mechanism lane steps to the previous or next occupied bucket in that lane; with All mechanisms selected it steps the whole machine. Deeper-rung lanes do not yet exist (§3.2): at the evidence rung this moves to the previous or next record; elsewhere it moves the analysis interval to the previous or next drawn bucket holding a record of the rung's focus. A zoomed view with nothing further pages on |

...
**Status:** reviewed product architecture and implementation blueprint, revision 18, 2026-09-22. M0 implementation has started; progress and measured limitations are tracked in `docs/IMPLEMENTATION-STATUS.md`. M0 must resolve the stated capture feasibility gates before dependent features are committed for delivery. This design document does not itself imply a capture benchmark or capability claim. Revision 18 freezes the broker request framing: a 32-byte header rejects payloads above 64 KiB before allocation, typed TLV fields have count/item bounds, unknown optional fields are skipped while unknown required fields are refused, every command request has a typed schema, and Hello negotiates optional versus required features explicitly. Response schemas, dispatch and OS-authenticated transport remain disabled.
...
The v1 frame has a fixed 32-byte header and a payload capped at 64 KiB before allocation. Typed TLV payloads cap fields at 64 and each value at 16 KiB; duplicate IDs, invalid widths/UTF-8/list counts, unknown required fields and truncated input are refused. Unknown optional fields are skipped and still consume the count/byte budget. Hello distinguishes requested optional feature bits from required bits. The request codec covers all seven commands and maps only bounded profile/override/quota/retention values; response schemas and dispatch must be frozen before the named-pipe listener is enabled.

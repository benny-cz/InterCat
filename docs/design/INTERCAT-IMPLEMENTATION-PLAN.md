...
Revision 86 clarifies the directory wording in step 2 after ADR-027: the broker-owned directory is the protected raw-evidence source, not a viewer-writable analysis session. The automatic user-owned derived session is still one action and needs no destination prompt. The current Desktop implementation shows a published L0-L2 overview; the channel, operation and record rungs remain explicitly unavailable until their real-session projections exist. This staged implementation does not satisfy the full first-run exit gate yet, nor is the three-second feedback budget measured.

...
**Status:** reviewed product architecture and implementation blueprint, revision 16, 2026-09-22. M0 implementation has started; progress and measured limitations are tracked in `docs/IMPLEMENTATION-STATUS.md`. M0 must resolve the stated capture feasibility gates before dependent features are committed for delivery. This design document does not itself imply a capture benchmark or capability claim. Revision 16 freezes the broker's transport-independent ownership semantics: prepared tokens are random, expiring, owner-bound and distinct from plan digests; durable intents precede runtime operations; duplicate request IDs replay one result; and partial stop milestones remain separately visible and retryable. The in-memory development store is not durability evidence and cannot enable live capture.

...
**The timeline follows the rung too.** The per-rung lanes of the table's timeline column are not built yet. Until they are, each rung's timeline draws in colour the records E reads from that rung:
...
dominant hue. This is data for the next rendering slice, not a claim that the Desktop already draws lanes.
...
| `[` / `]` | Previous / next observation in the selected lane or channel. Until the timeline has lanes (§3.2), at the evidence rung this is the previous or next record. At any other rung, the previous or next drawn bucket holding a record of the rung's focus, or of the machine, becomes the analysis interval. A zoomed view with nothing further pages on |

...
Revision 114 completes the timeline's rows of this table. It adds double-click zoom, pinch, `Shift`+arrow, horizontal and `Shift`+wheel pan, and `[`/`]`, and makes `+`/`-` zoom around the analysis interval. The pan and zoom rows are shared with the minimap's keyboard path. Still open:

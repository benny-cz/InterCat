...
accessibility/organization are not implied by this L0 result.
...
| `[` / `]` | Previous / next observation in the selected lane or channel is the target behavior. The current L0 mechanism rows are not selectable as a persistent lane focus, and deeper-rung lanes do not yet exist (§3.2): at the evidence rung this moves to the previous or next record; elsewhere it moves the analysis interval to the previous or next drawn bucket holding a record of the rung's focus, or of the machine. A zoomed view with nothing further pages on |

...
**Status:** reviewed product architecture and implementation blueprint, revision 17, 2026-09-22. M0 implementation has started; progress and measured limitations are tracked in `docs/IMPLEMENTATION-STATUS.md`. M0 must resolve the stated capture feasibility gates before dependent features are committed for delivery. This design document does not itself imply a capture benchmark or capability claim. Revision 17 freezes the broker ownership log and restart rules: append-only checksummed snapshots are flushed before success, a torn/corrupt tail falls back to the last complete frame, a nonempty store with no valid frame is refused, and recovery stops interrupted/expired ownership using a persisted session token rather than a matching name. Transport authentication and the real capture runtime remain mandatory before live use.

# InterCat implementation status — historical log through plan revision 103
...
row positions. A whole-extent viewport follows the new extent; a deliberate brush keeps its exact half-open time
range where retained. If retention clips or removes that range, or a focused entity disappears, the viewer stops at
the nearest surviving context and says what changed. It never moves an expired brush onto unrelated evidence. A
different session starts fresh, so stale intent cannot cross sessions.

Four new Desktop tests cover same-session continuity, a removed focus, an expired brush and the direct evidence jump.
...
## Previous slice: Desktop one-action Explore and saved-session opening
...
**Not yet the §3.1 exit gate:** the viewer has no real channel, operation or exact-record projection (L3-L5). The
3-second useful-feedback budget is unmeasured. The broker is
...
Next (as of this previous slice): preserve rung/viewport across generation changes; project channels, operations and evidence from a leased
...
`capture-finalization-v1` dependency is staged only with the last generation after the owned capture session stop
returns. It records the capture ID plus the provider-stop and callback-drain observations available at that point.
Intermediate generations do not carry it. Re-derivation, mirroring and compaction preserve it as evidence, and
derived-file retention cannot release it.

Broker restart recovery reopens only an **existing** protected per-capture directory; it never creates an empty one.
It verifies the store and prepared-plan source identity, decodes the finalization marker, checks that the committed
boundary names the expected journal length/digest, verifies the journal capture identity, and replays the boundary
journal through its terminal while matching the committed record count. Only then may it recover
`JournalFinalized`; `CallbacksDrained` is recovered only when the verified marker actually recorded it. Provider stop
remains independently confirmed against the exact durable ETW ownership. For older recordings, a verified final
coverage ledger remains a compatibility signal for journal finality, but it deliberately cannot invent callback-drain
evidence.

The follower now uses the finalization marker as the normal finished signal and treats the coverage ledger only as a
legacy signal. This closes a UX correctness bug: a clean capture whose ETW loss counters were unreadable can have
unknown coverage while still being unambiguously finished. A regression test mirrors such evidence with no coverage
ledger and expects the follower to finish from finalization evidence. Additional tests cover restart recovery of a
committed final publication and prove that a recovery open of a missing capture directory does not create evidence.

The implementation plan is revision 76 and `contracts/capture-finalization-v1.md` freezes this compatibility
boundary before broker live publication is enabled. Revision 76 also makes the next disk gate precise: the configured
reserve is an InterCat write-admission floor with separately calculated finalization headroom, not a claim that unrelated
processes cannot consume the volume between checks.

Verification (re-certified with a real toolchain): the slice as first written did not compile - a target-typed
`new()` was ambiguous between the two `StageCaptureFinalization` overloads and the legacy-ledger check called a
non-existent `DerivedGenerationBuilder.CoverageLedger` (now `SessionSegments.CoverageLedger`). Review also found that a
fresh stop request (new request ID) while a completion write was queued would call the runtime again and could
downgrade proven milestones; `StopOwnedAsync` now completes the queued result first, and the maintenance sweep retries
queued completions even when the runtime exposes no completion probe. Recovery skips store verification when both
journal and drain milestones are already durable. New tests cover the marker codec and its refusals, digest tamper
detection, absent/duplicate markers, retention/re-derivation/chunk-release preservation, foreign-capture refusal in
the builder, restart refusal of an unfinalized chunk / foreign marker / unreplayable journal, durable milestones
without ETW or directory access, and queued-completion reuse by new stop requests and status. All 719 tests pass in
Debug and Release. The broker executable remains disabled.

Next: enforce the free-disk reserve at the write/finalization boundary, then freeze and benchmark the prepared
live-publication cadence, wire the host maintenance/lease sweep loop, and finally exercise authenticated pipe +
protected root + crash/restart recovery with real elevated ETW.

## Previous slice: retryable stop-result persistence without runtime replay

The lifecycle coordinator now keeps a completed stop result in memory when its durable completion write fails. A
new maintenance API retries that exact ownership/request pair, and the existing autonomous-completion sweep retries
pending writes before scanning for newly completed captures. Authenticated status and lease renewal also retry a
pending completion for their own capture. A different owner still cannot trigger reconciliation.

The important invariant is that persistence retry does **not** call the capture runtime again. The runtime may already
have removed the active capture after proving provider stop, callback drain and final journal publication; replaying
`StopAsync` at that point follows the restart-reclaimer path and would downgrade those proven milestones to partial
evidence. Persistence failures now report the runtime's proven milestones while keeping durable lifecycle state at
`Stopping`, and a test fixture covers one failed completion write followed by a successful retry with exactly one
runtime stop.

The implementation plan is revision 74. It now explicitly separates the 4 Hz analytic/UI freshness target from the
more expensive journal-manifest publication cadence. Journal cadence must be a bounded prepared-plan setting,
returned in the effective summary and covered by the plan digest, rather than an arbitrary runtime interval. This
removes a sequencing ambiguity before live follow is enabled.

Verification: compiled and tested together with the finalization slice above (719 passing in Debug and Release).
Next: verify already-published final
journals during restart recovery, add the hard disk-reserve/write-boundary guarantee, define and benchmark the
prepared live-publication cadence, then wire a host loop that runs completion/persistence maintenance and lease
sweeps before elevated pipe-to-root crash/restart enablement.

## Previous slice: quota-safe live journal rollover

The evidence-only `LiveRecorder` can now publish intermediate journal generations while obeying one aggregate
`MaximumJournalBytes` allowance. Rollover accounts for all previously published journal bytes, the exact size of
the current chunk including buffered records, and the complete minimum next chunk including its terminal. If
the next chunk would not fit, it keeps the current one open for finalization rather than spinning on the due
timer. Every append uses the remaining allowance; the last generation still carries the coverage ledger.

A test opened a published generation as a reader while the pump was still active, then verified multiple
published journals together stayed under the 1 MiB cap after quota stop. All 705 tests pass in Debug and Release.
The broker runtime still passes no publication interval. Its cadence needs a bounded, measured policy frozen in
the prepared effective summary before live follow is enabled; a too-frequent interval repeatedly verifies a
growing manifest and could harm capture throughput. The executable remains disabled for the pending host sweep,
persistence-retry, hard disk-reserve and elevated pipe/restart gates.

## Previous slice: durable autonomous-stop reconciliation

The evidence runtime now exposes a read-only completion probe. The lifecycle coordinator detects a duration,
journal or disk-triggered end, writes an `AutonomousStop` intent to its durable ownership log, obtains the runtime's
separate stop milestones and commits the result. An authenticated status query reconciles before answering; owner
lease renewal cannot extend a capture whose runtime has already ended. The host has a no-client sweep API for the
same operation. A different owner cannot trigger reconciliation by asking for status.

File-backed tests confirm status-triggered and proactive reconciliation survive reopen without replaying a second
stop. An integrated test runs the evidence runtime through a one-second duration and verifies the resulting
`Closed` state, milestones and request in the durable store. All 704 tests pass in Debug and Release.

The executable remains disabled. Its future host must actually schedule the sweep; a stop-result persistence
failure can leave a pending intent that only startup recovery currently retries; and startup recovery does not yet
verify an already-committed final journal after a broker crash. The free-disk reserve is still observed once per
second, not guaranteed at each write, and bounded captures cannot publish chunks for live follow yet. Next:
make the host sweep/persistence retry safe, verify final journals during crash recovery, add quota-aware live
chunk rollover and a hard disk reserve, then run elevated pipe-to-root crash/restart tests before enablement.

## Previous slice: evidence-only broker runtime and exact journal quota
...
| IC-014 broker protocol, leases and idempotency | In progress: protocol/authenticated pipe, protected root, preparation/ownership/recovery and evidence runtime are composed; executable remains disabled | `BrokerPrepareCompiler` freezes locally compiled allowlisted plans; prepared tokens bind to authenticated SID/logon and are single-start; checksummed ownership snapshots preserve exact idempotency, leases and independent stop milestones. The local-only first-instance pipe impersonates the client and authenticates SID/logon/integrity/elevation; the protected broker root is handle-validated against reparse/final-path/volume/security substitution. The runtime binds a prepared plan to exact-name ETW ownership and a quota-aware admitted journal, publishes explicit `capture-finalization-v1` last-publication evidence, and restart recovery verifies the existing store, plan identity, boundary journal and terminal replay before promoting journal/callback milestones. Autonomous completion and failed completion persistence have retryable maintenance paths that never replay an already-finished runtime. Current source changes add three regression facts beyond the previous archive; this environment has no .NET SDK, so the prior archive's 705-test result is historical rather than re-certified. Remaining enablement gates: write-boundary free-disk/finalization reserve, measured prepared journal-publication policy, host maintenance/lease scheduling, then elevated authenticated pipe/root crash-restart proof and desktop live-follow composition. |
...
IC-011 and IC-012 are complete for the source evidence the repository actually has. Explore and Focused TCP compile enforceable metadata policies; Content compiles a reviewable request contract and an enforceable refusal. IC-014 has an evidence-only ETW/journal runtime, durable autonomous-stop and stop-result-persistence maintenance paths, quota-aware recorder rollover, and restart-verifiable final-publication evidence behind the disabled executable. The remaining enablement gates are write-boundary free-disk/finalization reserve enforcement, a measured prepared journal-publication policy, the host maintenance/lease loop and elevated authenticated pipe/root crash-restart proof. IC-013 imports a real standalone ETL end to end and unelevated into a published session. M1's durable commit protocol, manifests and derived segments are in; a reader holds a generation and its dependencies as one lease, and retention publishes what it released rather than merely deleting it. `metrics-v1` answers counts, byte totals and rates over a whole session, refuses what means nothing and reports what cannot be answered. The store still needs the checkpoint §20.2 owes at a rolling boundary, and live viewer integration still waits on those layers.
